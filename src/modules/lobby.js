...
export function renderLobby(root,{openGameSheet}){
  ...
  root.innerHTML=`
    <section class="room-card room-summary-card" aria-label="当前派对房间">
      <div class="room-top"><div><span class="eyebrow">Midnight Game Hall</span><h1>${escapeHtml(state.roomName)}</h1><p class="room-subcopy">私人派对空间已就绪，选择一个房间开始今晚的游戏。</p></div><button class="text-button room-manage" data-players>管理玩家 <span aria-hidden="true">›</span></button></div>
      ...
      <div class="room-players">${active.slice(0,5).map(player=>`<span class="avatar" title="${escapeHtml(player.name)}">${escapeHtml(initials(player.name))}</span>`).join('')}${active.length>5?`<span class="avatar more">+${active.length-5}</span>`:''}<span class="ready-chip"><i></i>今晚开局</span></div>
    </section>
    <section class="lobby-section"><div class="section-title"><div><span class="eyebrow">Game Rooms</span><h2>选择游戏房间</h2></div><span>${games.length} 种玩法</span></div><div class="game-grid">${games.map((game,index)=>card(game,index)).join('')}</div></section>`;
...
function card(game,index){return `<button class="game-card" data-game="${game.id}" style="--accent:${game.color}" aria-label="${escapeHtml(game.title)}，${game.minPlayers}到${game.maxPlayers}人"><span class="game-glow"></span><span class="game-card-top"><span class="game-icon">${icon(game.icon)}</span><span class="game-index">${String(index+1).padStart(2,'0')}</span></span><strong>${escapeHtml(game.title)}</strong><p>${escapeHtml(game.shortDescription)}</p><span class="game-room-label">${escapeHtml(roomLabels[game.id]||'午夜游戏房')}</span><span class="game-card-footer"><span class="game-meta">${game.minPlayers}–${game.maxPlayers} 人</span>${game.supportsAdult?'<span class="advanced-chip">成人进阶</span>':''}</span></button>`}
